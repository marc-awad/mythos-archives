...
async function testAuthServiceCommunication() {
    ...
    try {
        const isHealthy = await auth_service_1.authServiceClient.healthCheck();
        if (isHealthy) {
    ...
    try {
        await auth_service_1.authServiceClient.verifyToken("invalid_token_xyz");
    ...
    try {
        ...
        const expiredToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwiZW1haWwiOiJ0ZXN0QHRlc3QuY29tIiwidXNlcm5hbWUiOiJ0ZXN0dXNlciIsInJvbGUiOiJVU0VSIiwiaWF0IjoxNzAwMDAwMDAwLCJleHAiOjE3MDAwMDAwMDF9.xxx";
        await auth_service_1.authServiceClient.verifyToken(expiredToken);
    ...
    console.log("💡 Pour tester avec un vrai token, vous devez d'abord vous connecter\n");
    console.log("Étapes:");
