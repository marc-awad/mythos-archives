...
async function testServerConnection() {
    ...
    // Test Auth Service
    try {
        const authResponse = await axios_1.default.get(AUTH_URL, {
            timeout: 5000,
            validateStatus: () => true,
        });
        log(`✓ Auth service accessible sur ${AUTH_URL} (status: ${authResponse.status})`, colors.green);
    }
    catch (error) {
    ...
    // Test Lore Service
    try {
        const loreResponse = await axios_1.default.get(LORE_URL, {
            timeout: 5000,
            validateStatus: () => true,
        });
        log(`✓ Lore service accessible sur ${LORE_URL} (status: ${loreResponse.status})`, colors.green);
        return true;
    }
    catch (error) {
        if (error.code === "ECONNREFUSED") {
...
async function testGetToken() {
    ...
    try {
        ...
        const response = await axios_1.default.post(`${AUTH_URL}/auth/login`, {
        ...
        if (response.status === 200 && response.data.data?.token) {
            ...
            log(`  Utilisateur: ${response.data.data.user?.email || "N/A"}`, colors.green);
            log(`  Rôle: ${response.data.data.user?.role || "N/A"}`, colors.green);
            if (response.data.data.user?.role !== "EXPERT" &&
                response.data.data.user?.role !== "ADMIN") {
                log(`  ⚠️  ATTENTION: L'utilisateur n'est pas EXPERT/ADMIN`, colors.yellow);
                log(`     Les tests de création vont probablement échouer`, colors.yellow);
            }
            return true;
        }
        else {
        ...
        if (error.code === "ECONNREFUSED") {
            ...
            log(`  Message: ${error.response.data?.message || "Aucun message"}`, colors.yellow);
        }
        ...
        log("     Utilise l'endpoint POST /auth/register avec role: EXPERT", colors.yellow);
        return false;
    }
}
/**
...
async function testCreateCreature() {
    ...
    try {
        ...
        const response = await axios_1.default.post(API_URL, newCreature, {
        ...
        if (error.response) {
            log(`  Status: ${error.response.status}`, colors.yellow);
            log(`  Message: ${error.response.data?.message || "Aucun"}`, colors.yellow);
            if (error.response.status === 403) {
                log("  ⚠️  L'utilisateur n'a pas le rôle EXPERT ou ADMIN", colors.yellow);
            }
...
async function testGetCreatureById() {
    ...
    try {
        ...
        const response = await axios_1.default.get(`${API_URL}/${creatureId}`, {
            timeout: 10000,
            validateStatus: (status) => status < 500,
        });
        if (response.status === 200) {
...
async function testPagination() {
    ...
    try {
        const response = await axios_1.default.get(`${API_URL}?page=1&limit=5`, {
            timeout: 10000,
            validateStatus: (status) => status < 500,
        });
        if (response.status === 200) {
...
async function testSearch() {
    ...
    try {
        const response = await axios_1.default.get(`${API_URL}?search=Test`, {
            timeout: 10000,
            validateStatus: (status) => status < 500,
        });
        if (response.status === 200) {
            ...
            if (response.data.creatures && response.data.creatures.length > 0) {
                response.data.creatures.forEach((c, i) => {
                    log(`    ${i + 1}. ${c.name}`);
                });
            }
            return true;
...
async function testCreateWithoutAuth() {
    ...
    try {
        ...
        log(`✗ PROBLÈME: La créature a été créée sans authentification!`, colors.red);
        return false;
    }
    catch (error) {
...
async function runAllTests() {
    ...
    if (await testGetToken()) {
        ...
        results.failed++;
        log("\n⚠️  Tests 2-4 ignorés car l'authentification a échoué", colors.yellow);
        results.failed += 2;
    }
    // Test 3: Lecture de toutes les créatures (ne dépend pas de l'auth)
    if (await testGetAllCreatures())
        results.passed++;
    else
        results.failed++;
    // Test 5: Pagination
    ...
    if (await testSearch())
        ...
        results.failed++;
    // Test 7: Sécurité
    if (await testCreateWithoutAuth())
        results.passed++;
    else
        results.failed++;
    // Résumé
    ...
    log(`📈 Taux de réussite: ${((results.passed / (results.passed + results.failed)) *
        100).toFixed(1)}%`, results.failed === 0 ? colors.green : colors.yellow);
    log("=".repeat(60), colors.blue);
    if (results.failed === 0) {
        log("\n🎉 Tous les tests sont passés avec succès!", colors.green);
    }
    else {
        log("\n⚠️  Certains tests ont échoué. Vérifie les erreurs ci-dessus.", colors.yellow);
    }
}
// Lancer les tests
