...
class TestimonyController {
    ...
    async createTestimony(req, res, next) {
        try {
            ...
            // Créer le témoignage
            const testimony = await testimony_service_1.default.createTestimony({ creatureId, description }, req.user.id);
            res.status(201).json({
                success: true,
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes("ID de créature invalide") ||
     ...
     * Route publique
     */
    async getTestimonyById(req, res, next) {
        try {
            const { id } = req.params;
            if (!id) {
                res.status(400).json({
            ...
            const testimony = await testimony_service_1.default.getTestimonyById(id);
            res.status(200).json({
                success: true,
    ...
    async validateTestimony(req, res, next) {
        try {
            ...
            const testimony = await testimony_service_1.default.validateTestimony(id, req.user.id, req.user.role);
            res.status(200).json({
                success: true,
            ...
            if (error instanceof Error) {
                ...
                if (error.message === "Vous ne pouvez pas valider votre propre témoignage") {
                    res.status(403).json({
                        success: false,
                        message: error.message,
                    });
    ...
    async rejectTestimony(req, res, next) {
        try {
            ...
            const testimony = await testimony_service_1.default.rejectTestimony(id, req.user.id);
            res.status(200).json({
                success: true,
            ...
            if (error instanceof Error) {
                ...
                if (error.message === "Vous ne pouvez pas rejeter votre propre témoignage") {
                    res.status(403).json({
                        success: false,
                        message: error.message,
                    });
     ...
     */
    async getMyTestimonies(req, res, next) {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
            ...
            const testimonies = await testimony_service_1.default.getTestimoniesByAuthor(req.user.id);
            res.status(200).json({
                success: true,
    ...
    async deleteTestimony(req, res, next) {
        try {
            ...
            await testimony_service_1.default.softDeleteTestimony(id, req.user.id);
            res.status(200).json({
                success: true,
    ...
    async restoreTestimony(req, res, next) {
        try {
            ...
            const testimony = await testimony_service_1.default.restoreTestimony(id, req.user.id);
            res.status(200).json({
                success: true,
